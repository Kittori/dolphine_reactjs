  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      try {
        ...
        const itemsWithFishData = await Promise.all(response.data.map(async (item) => {
                        ...
                        <CardBody>
                          <div className="d-flex justify-content-between">
                            <div className="d-flex flex-row align-items-center">
                              <div>
                                <CardImg
                                  src={item.fishID.image ? require(`../assets/img/aquarium/${item.fishID.image}`) : ''}
                                  className="rounded-3"
                                  style={{ width: "80px", paddingRight: "1rem" }}
                                  alt="Shopping item"
                                />
                              </div>
                              <div className="ms-3 mb-0" style={{ width: "80px", paddingLeft: ".5rem", paddingTop: ".7rem" }}>
                                <h5>{item.fishID.name}</h5>
                              </div>
                              <div className="col-8">
                                <p className="small mb-0">{truncateDescription(item.fishID.description, 50)}</p>
                              </div>
                            ...
                            <div className="d-flex flex-row align-items-center">
                              ...
                              <div style={{ width: "80px" }}>
                                <h7 className="mb-0">{formatNumber(item.fishID.price)} VNĐ</h7>
                              </div>
                              <a onClick={() => handleRemoveItem(item.fishID._id)} style={{ color: "#cecece" }}>
                  ...
                  <Col lg="5">
                    <Card className="bg-info text-white rounded-3">
                      <CardBody>
                        ...
                        <Form className="mt-4">
                          <FormGroup>
                            <Label for="cardName" className="form-label">Cardholder's Name</Label>
                            <Input id="cardName" type="text" size="lg" placeholder="Cardholder's Name" className="form-control-lg" />
                          </FormGroup>
                          <FormGroup>
                            <Label for="cardNumber" className="form-label">Card Number</Label>
                            <Input id="cardNumber" type="text" size="lg" placeholder="1234 5678 9012 3457" className="form-control-lg" minLength="19" maxLength="19" />
                          </FormGroup>
                          <Row className="mb-4">
                            <Col md="6">
                              <FormGroup>
                                <Label for="cardExpiration" className="form-label">Expiration</Label>
                                <Input id="cardExpiration" type="text" size="lg" placeholder="MM/YYYY" className="form-control-lg" minLength="7" maxLength="7" />
                              </FormGroup>
                            </Col>
                            <Col md="6">
                              <FormGroup>
                                <Label for="cardCvv" className="form-label">Cvv</Label>
                                <Input id="cardCvv" type="text" size="lg" placeholder="&#9679;&#9679;&#9679;" className="form-control-lg" minLength="3" maxLength="3" />
                              </FormGroup>
                            </Col>
                          </Row>
                        </Form>
